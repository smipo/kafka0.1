...
import kafka.clients.producer.Partitioner;
...
public class DefaultPartitionerTest {
    ...
    private String topic = "kafka/test";
    ...
    public void testKeyPartitionIsStable() {
        int partition = partitioner.partition("kafka/test",  null, keyBytes, null, null, cluster);
        assertEquals("Same key should yield same partition", partition, partitioner.partition("kafka/test", null, keyBytes, null, null, cluster));
    }
    ...
    public void testRoundRobinWithUnavailablePartitions() {
        ...
        int countForPart2 = 0;
        for (int i = 1; i <= 100; i++) {
            int part = partitioner.partition("kafka/test", null, null, null, null, cluster);
