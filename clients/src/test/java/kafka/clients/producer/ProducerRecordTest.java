...
public class ProducerRecordTest {
    ...
    public void testEqualsAndHashCode() {
        ProducerRecord<String, Integer> producerRecord = new ProducerRecord<>("kafka/test", 1, "key", 1);
        assertEquals(producerRecord, producerRecord);
        assertEquals(producerRecord.hashCode(), producerRecord.hashCode());

        ProducerRecord<String, Integer> equalRecord = new ProducerRecord<>("kafka/test", 1, "key", 1);
        ...
        ProducerRecord<String, Integer> partitionMismatch = new ProducerRecord<>("kafka/test", 2, "key", 1);
        assertFalse(producerRecord.equals(partitionMismatch));

        ProducerRecord<String, Integer> keyMisMatch = new ProducerRecord<>("kafka/test", 1, "key-1", 1);
        assertFalse(producerRecord.equals(keyMisMatch));

        ProducerRecord<String, Integer> valueMisMatch = new ProducerRecord<>("kafka/test", 1, "key", 2);
    ...
    public void testInvalidRecords() {
        ...
        try {
            new ProducerRecord<>("kafka/test", 0, -1L, "key", 1);
        ...
        try {
            new ProducerRecord<>("kafka/test", -1, "key", 1);
